/**
 * radio_controller.ino - Physical Controller for Retro Internet Radio
 */
...
// HELPER FUNCTIONS
 ...
 // Writes text to Line 1 or 2, auto-padded to 16 chars to clear ghost letters
...
// Reads potentiometer and emits STATION:x once dial settles
void checkTuner() {
  int raw = analogRead(PIN_POT_TUNER);

  if (abs(raw - lastRawAdc) > ADC_JITTER_DEADBAND) {
    lastRawAdc = raw;
...
// Reads incoming commands from Pi over Serial
void checkSerial() {
...
void setup() {
  ...
  // Initial read of the tuner
  lastRawAdc = analogRead(PIN_POT_TUNER);
  currentStation = map(lastRawAdc, 0, 1024, 0, TOTAL_STATIONS);
